...
def motor_run(when_open, when_close, price_close_long, price_close_short, price_open_long, price_open_short, order_out, df_dict, open_positions_long, open_positions_short, account_size_fw, available, tickers_list_b, delta, position_size_st, short_selling, time_stop, stop_selection, trailing, comision, s_l, t_p):
    ...
    for i in velas.index:
        ...
        for ticker in open_positions_long.copy():
            if ticker not in tickers_list_b:  # esto puede ocurrir en walk_forward con cambio de cartera
                ...
                available += (open_positions_long[ticker]['Position_Size'] + closed_positions.loc[trade_id,'Profit/Loss_$'])
                trade_id += 1
                del open_positions_long[ticker]

            elif (time_stop > 0) and (open_positions_long[ticker]['Bars_In'] >= time_stop):
                ...
                account_size_fw += closed_positions.loc[trade_id,'Profit/Loss_$']
                closed_positions.loc[trade_id,'Account_Size'] = account_size_fw
                available += (closed_positions.loc[trade_id,'Position_Size'] + closed_positions.loc[trade_id,'Profit/Loss_$'])
                trade_id += 1
                del open_positions_long[ticker] 
        ...
        for ticker in open_positions_short.copy():
            ...
            elif (time_stop > 0) and (open_positions_short[ticker]['Bars_In'] >= time_stop):
                closed_positions.loc[trade_id,'Close_Date'] = df_dict[ticker].loc[i,'Date'] 
                closed_positions.loc[trade_id,'Ticker'] = ticker
                closed_positions.loc[trade_id,'Direction'] = 'short'
                closed_positions.loc[trade_id,'Open_Date'] = open_positions_short[ticker]['Open_Date']
...
def unit_backtest(source, setting, account_sets, strategy, strategy_set, contador):
...
def mp_backtest(source, settings, account_sets, strategy, strategy_sets, save_to):
    ...
    with concurrent.futures.ProcessPoolExecutor() as e: 
        ...
        for setting in setting_combos:
            for strategy_set in strat_combos:
                Id += 1
                p1 = e.submit(unit_backtest, source, setting, account_sets, strategy, strategy_set, Id)
...
def step_forward(source, settings, account_sets, strategy, strategy_set):
...
def mp_walk_forward(source, settings, account_sets, strategy, strategy_sets, steps, save_to):
    output_metrics = pd.DataFrame()
    output_metrics_long = pd.DataFrame()
    output_metrics_short = pd.DataFrame()
    if save_to != None:
    ...
    for step in steps:
        ...
        while fin_bt < fecha_hasta:
            ...
            metricas_bt, results_dict_bt = mp_backtest(source, settings, account_sets, strategy, strategy_sets, save_to=None)
            ...
            fw_closed_positions, fw_velas, fw_acc_sets = step_forward(source, settings_fw, acc_sets_fw, strategy, strat_sets_fw)
